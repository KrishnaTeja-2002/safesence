...
export default function ChatBot() {
  ...
  return (
    <>
      {/* Floating Chat Button - Improved Contrast */}
      {!isOpen && (
        <button
          onClick={() => setIsOpen(true)}
          className={`fixed bottom-6 right-6 w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-xl z-[9999] group ${
            darkMode
        ...
        >
          ...
          <span className={`absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full border-2 ${
            darkMode ? 'bg-green-400 border-blue-600' : 'bg-green-400 border-blue-600'
          }`} style={{ boxShadow: '0 0 4px rgba(34, 197, 94, 0.6)' }}></span>
        </button>
      )}
      ...
      {isOpen && (
        <div
          className={`fixed bottom-8 right-8 w-[420px] h-[650px] rounded-3xl shadow-2xl flex flex-col overflow-hidden z-[9999] border-2 transition-all duration-300 ${
            darkMode
              ? 'bg-slate-950 border-blue-800/50'
              : 'bg-white border-blue-200'
          }`}
          style={{
            boxShadow: darkMode 
              ? '0 25px 50px -12px rgba(0, 0, 0, 0.6), 0 0 0 1px rgba(59, 130, 246, 0.1)' 
              : '0 25px 50px -12px rgba(0, 0, 0, 0.3), 0 0 0 1px rgba(59, 130, 246, 0.1)'
          }}
        >
          {/* Header with Gradient - Better Contrast */}
                ...
                <div
                  className={`max-w-[85%] rounded-2xl px-5 py-3.5 shadow-lg transition-all duration-200 hover:shadow-xl ${
                    message.role === 'user'
                      ...
                      : 'bg-white text-slate-900 border border-slate-300 mr-8'
                  }`}
                  style={{
              ...
              <div className="flex justify-start animate-fade-in">
                ...
                >
                  <div className="flex space-x-2 items-center">
                    <div className={`w-2.5 h-2.5 rounded-full animate-bounce ${darkMode ? 'bg-blue-400' : 'bg-blue-500'}`} style={{ animationDelay: '0ms' }}></div>
                    <div className={`w-2.5 h-2.5 rounded-full animate-bounce ${darkMode ? 'bg-indigo-400' : 'bg-indigo-500'}`} style={{ animationDelay: '150ms' }}></div>
                    <div className={`w-2.5 h-2.5 rounded-full animate-bounce ${darkMode ? 'bg-blue-400' : 'bg-blue-500'}`} style={{ animationDelay: '300ms' }}></div>
                    <span className={`ml-2 text-xs ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>AI is thinking...</span>
                  </div>
                </div>
              </div>
            )}
          ...
          >
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  className={`w-full px-4 py-3 rounded-2xl resize-none focus:outline-none focus:ring-2 transition-all text-sm ${
                    darkMode
                      ? 'bg-slate-800 text-white placeholder-slate-400 focus:ring-blue-500 border border-slate-700'
                      : 'bg-slate-50 text-slate-900 placeholder-slate-500 focus:ring-blue-500 border border-slate-300'
                  } ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                />
                {/* Character Counter */}
                <div className={`absolute bottom-2 right-3 text-xs ${darkMode ? 'text-slate-500' : 'text-slate-500'}`}>
                  {input.length}/500
                </div>
              </div>
              <button
                ...
                className={`px-5 py-3 rounded-2xl font-medium transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-xl ${
                  isLoading || !input.trim()
                    ...
                    : darkMode
                    ? 'bg-gradient-to-br from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white transform hover:scale-105'
                    : 'bg-gradient-to-br from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white transform hover:scale-105'
                }`}
              >
                <svg
            ...
            <p className={`text-xs mt-2 text-center ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
              ...
              <kbd className={`px-2 py-0.5 rounded text-xs font-mono ml-1 ${
                darkMode ? 'bg-slate-700 text-slate-200' : 'bg-slate-200 text-slate-700'
