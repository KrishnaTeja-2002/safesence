...
export class DatabaseClient {
  ...
  async createAuthUserFromGoogle({ email, name, picture, googleId }) {
    ...
    const rows = await this.prisma.$queryRaw`
      ...
      RETURNING id, email, created_at, email_confirmed_at, confirmed_at
    `;
    ...
    if (newUser) {
      ...
      await this.upsertUserPreferences(newUser.id, {
        tempScale: 'F',
        showTemp: true,
        showHumidity: false,
        showSensors: true,
  ...
  async canUserWriteToSensor(userId, sensorId, deviceId, userEmail = null) {
    ...
    // Since user_id is often null, we need to check by both userId and email
    // Use raw SQL to handle null user_id properly
    const invitation = await this.prisma.$queryRaw`
      SELECT sensor_id, email, role, user_id, status
      FROM public.team_invitations 
      WHERE sensor_id = ${sensorId}
      AND status = 'accepted' 
      AND (user_id = ${userId}::uuid OR LOWER(email) = LOWER(${userEmail || ''}))
      AND (LOWER(role) LIKE '%admin%' OR LOWER(role) LIKE '%full%')
      LIMIT 1
    `;
